...
class Connection(ConnectionBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.jailspec, self.jailhost = self.host.split('@', 1)
    ...
    def get_jail_connector(self):
        if self.connector == None:
            code, _, _= self._exec_command("which -s jailme")
            if code != 0:
    ...
    def _exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        if in_data:
            cmd = self._build_command('ssh', self.jailhost, cmd)
        else:
            cmd = self._build_command('ssh', '-tt', self.jailhost, cmd)
